...
//    {
//        static constexpr Dest_t m_value = 1. / (std::uintmax_t{ 1 } << F);
//    };
//
//
...
//    {
//        using T = fixed<I, F, sign_t>::m_value_type;
//
//    public:
//        using fixed_type = fixed<I, F, sign_t>;
//        using array_type = std::array<T, N>;
...
namespace util {
	...
	template<std::size_t decimal_bits, std::integral underlying_type = std::int32_t>
	class FixedFunctionFloat {

	private:
		static const unsigned int DECIMAL_BITS = decimal_bits;
		static const unsigned int SCALING_FACTOR = 2 << DECIMAL_BITS;
		static const unsigned int DECIMAL_MASK = (2 << DECIMAL_BITS) - 1;

	public:
		struct raw_t {};
		using type = FixedFunctionFloat<decimal_bits, underlying_type>;

		constexpr FixedFunctionFloat() = default;
		
		constexpr FixedFunctionFloat(const type& value) : m_value{ value.m_value } {}
		
		constexpr FixedFunctionFloat(const underlying_type& value, raw_t r) : m_value{ value } {}
				
		template<typename T>
			requires (
			std::is_integral_v<T> &&
			std::is_signed_v<T> == std::is_signed_v<underlying_type> )
		constexpr FixedFunctionFloat(T int_value) : m_value((int_value << DECIMAL_BITS)) {}
		...
		constexpr FixedFunctionFloat(T int_value) : m_value((static_cast<underlying_type>(int_value) << DECIMAL_BITS)) {}

		template<std::floating_point T>
		constexpr FixedFunctionFloat(T float_value) : m_value(
			static_cast<underlying_type>(float_value * static_cast<T>(underlying_type{ 1 } << DECIMAL_BITS))
		) {}
		

		constexpr FixedFunctionFloat & operator += (const FixedFunctionFloat other) {
		...
		constexpr auto operator <=>(const FixedFunctionFloat&) const = default; 
		
		template<typename T>
		constexpr auto as() const {
			if constexpr (std::is_integral_v<T>) {
				return m_value >> DECIMAL_BITS;
			}
			else {
				T fixed = m_value >> DECIMAL_BITS;
				fixed += static_cast<T>(m_value & DECIMAL_MASK) / SCALING_FACTOR;
				return fixed;
			}
		...
		friend constexpr type operator * (const type& lhs, const FixedFunctionSign sign) {
			if constexpr (std::is_signed_v<underlying_type>) {
				...
				static_assert("Can only used signs on signed values");
			}
		}

	
	private:
		/*constexpr int_type make_m_value(int_type iT, dec_type dV) {
			return (iT << DECIMAL_BITS) | (dV & DECIMAL_MASK);
		}*/
